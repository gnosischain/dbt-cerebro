...
- [vendor-published-view-config-hash-gate](vendor-published-view-config-hash-gate.md) `remediated` —
  a service's "published" view gated on the target's current config hash
  hid 2,207 days of GNO supply after an indexer config-model change; dbt now selects the
  published attempt over the base tables (PR #77, deployed 2026-09-09).
