...
**Post-fix dev validation (playground_max, 2026-06-26).** Dev staging stops at `2026-05-26`, so May is dev's current month; the incremental run with the fixed code rebuilt it. Result: `int_execution_cow_trades` May = `0.0%` NULL solver across all `42,136` trades (pre-fix the incremental path would leave `05-01..05-23` NULL); `fct_execution_cow_daily` May shows `active_solvers` `7–12` every day (not `0`) and `cow_ratio`/`num_cow_batches` `= 0` every day — the true value (real pure-CoW ~0 on Gnosis since 2022), vs the pre-fix spurious `0.06–0.18`. Incremental path confirmed fixed; prod will repair June identically on its next cron run.
