...
raw_deltas AS (
   ...
   AND 
    toStartOfMonth(toDate(e.block_timestamp)) >= (
      ...
      FROM `dbt`.`int_execution_pools_balancer_v2_daily` AS x1
      WHERE 1=1 
    )
    AND toDate(e.block_timestamp) >= (
      ...
      FROM `dbt`.`int_execution_pools_balancer_v2_daily` AS x2
      WHERE 1=1 
    )
...
prev_balances AS (
    ...
    FROM `dbt`.`int_execution_pools_balancer_v2_daily` FINAL
    WHERE date = (SELECT max(date) FROM `dbt`.`int_execution_pools_balancer_v2_daily` FINAL)
),
