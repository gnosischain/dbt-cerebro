...
FROM `dbt`.`int_execution_gnosis_app_conversions` c
INNER JOIN `dbt`.`int_execution_gnosis_app_user_events_unified` e
...
WHERE 1=1
    ...
    AND 
      ...
      AND toDate(c.conversion_date) >= (
        ...
        FROM `dbt`.`fct_execution_gnosis_app_journeys_60d` AS x2
        WHERE 1=1 
      )
