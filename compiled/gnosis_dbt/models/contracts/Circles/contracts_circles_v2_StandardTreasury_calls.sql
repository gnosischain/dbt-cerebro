...
WITH
  tx AS (
    SELECT * FROM (
      ...
      WHERE replaceAll(lower(to_address),'0x','') = '08f90ab73a515308f03a718257ff9887ed330c6e'
          ...
          AND block_timestamp >
              (SELECT coalesce(max(block_timestamp), '1970-01-01') FROM `dbt`.`contracts_circles_v2_StandardTreasury_calls`)
