...
WITH
  tx AS (
    SELECT * FROM (
      ...
      WHERE replaceAll(lower(to_address),'0x','') = '29b9a7fbb8995b2423a71cc17cf9810798f6c543'
          ...
          AND block_timestamp >
              (SELECT coalesce(max(block_timestamp), '1970-01-01') FROM `dbt`.`contracts_circles_v1_Hub_calls`)
