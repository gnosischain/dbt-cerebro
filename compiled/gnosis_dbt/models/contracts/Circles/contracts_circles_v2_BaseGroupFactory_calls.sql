...
WITH
  ...
  tx AS (
    SELECT * FROM (
      ...
      FROM src
      WHERE replaceAll(lower(to_address),'0x','') = 'd0b5bd9962197beac4cba24244ec3587f19bd06d'
          ...
          AND block_timestamp >
              (SELECT coalesce(max(block_timestamp), '1970-01-01') FROM `dbt`.`contracts_circles_v2_BaseGroupFactory_calls`)
