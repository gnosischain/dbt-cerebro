{{
  config(
    materialized='view',
    tags=['production','revenue','revenue_holdings','api:revenue_holdings_cohorts_monthly','granularity:monthly'],
    meta={
      "api": {
        "methods": ["GET"],
        "allow_unfiltered": true,
        "parameters": [
          {"name": "symbol",     "column": "symbol", "operator": "=",  "type": "string", "description": "Token symbol (EURe, USDC.e, BRLA, ZCHF)"},
...
FROM {{ ref('fct_revenue_holdings_cohorts_monthly') }}
