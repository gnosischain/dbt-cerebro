{{
    config(
        materialized='view',
        tags=['production','execution','tier1','api:yields_pools', 'metric:lp_activity', 'granularity:daily']
    )
}}
...
{#-
  ...
  Reuses pool labels and top-pool filtering from fct_execution_yields_pools_daily.
-#}
...
lp_by_pool AS (
    ...
    FROM {{ ref('int_execution_yields_pools_lps_daily') }} d
