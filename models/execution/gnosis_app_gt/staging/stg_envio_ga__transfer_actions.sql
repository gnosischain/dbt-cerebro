{{ config(
    ...
    order_by='(participant, transfer_type)',
    ...
    meta={
        ...
        'grain': 'participant_x_transfer_type',
        'guard': 'STRETCH — full-scans envio_ga.transfer (108M, no partition/block pruning possible). Reduced to a compact per-(participant, transfer_type) aggregate. Do NOT microbatch (each batch re-scans the whole table); built once per run.'
    }
) }}
