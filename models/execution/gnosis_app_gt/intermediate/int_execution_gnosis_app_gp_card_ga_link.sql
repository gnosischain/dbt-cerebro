{{ config(
    ...
    tags=['production', 'execution', 'gnosis_app_gt', 'gpay', 'internal_only', 'privacy:tier_internal'],
    meta={
        'expose_to_mcp': false,
        'privacy_tier': 'internal',
        'api': {'exclude_from_api': true},
        'grain': 'canonical_card',
        'guard': 'Authoritative, MODULE-AGNOSTIC Gnosis Pay card -> Gnosis App LINK (one row per canonical card). Union of the Mixpanel `pay`-profile bridge (app-truth) and the three on-chain signals in int_execution_gnosis_app_gt_card_owner (Delay controller / cashback / top-up funder). Replaces the DelayModule-only onboarding heuristic that broke in the June-2026 RolesModule migration: this is a LINK (a GA account controls/funds this card), NOT the onboarded_via_ga-vs-imported CLASS (which is on-chain-unrecoverable post-migration: 0/68,680 Role owners and 0/34,340 Safe-setup owners are GA users). Internal (raw card + pseudonym) — only the aggregate fct_/api_ ..._gp_card_ga_link_daily views are exposed.'
    }
) }}
...
mixpanel_pay AS (
    ...
    FROM {{ ref('int_mixpanel_ga_gpay_pay_bridge') }} pb
...
onchain AS (
    ...
    FROM {{ ref('int_execution_gnosis_app_gt_card_owner') }}
...
cashback_dates AS (
    ...
    FROM {{ ref('stg_envio_ga__cashbacks') }} cb
