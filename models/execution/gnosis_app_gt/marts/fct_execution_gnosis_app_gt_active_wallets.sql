{{ config(
    ...
    order_by='(period_type, period_start)',
    tags=['production', 'execution', 'gnosis_app_gt', 'mart'],
    meta={'grain': 'period_type_x_period_start'}
) }}
...
WITH first_seen AS (
    ...
    FROM {{ ref('stg_envio_ga__wallet_activity_daily') }}
    GROUP BY address
),
acts AS (
    SELECT w.address, w.activity_date, w.is_app_tagged_day, f.first_day, f.first_day_app
    FROM {{ ref('stg_envio_ga__wallet_activity_daily') }} w
