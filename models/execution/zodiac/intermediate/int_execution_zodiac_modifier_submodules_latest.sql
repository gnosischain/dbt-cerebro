{{
  config(
    ...
    order_by='(modifier_address, submodule_address)',
...
WITH submodule_state AS (
    ...
    FROM {{ ref('int_execution_zodiac_modifier_module_events') }}
...
registry AS (
    ...
    FROM {{ ref('contracts_zodiac_modules_registry') }}
),

safes AS (
    SELECT address FROM {{ ref('contracts_safe_registry') }}
),

gp_safes AS (
    SELECT address FROM {{ ref('int_execution_gpay_wallets') }}
)
