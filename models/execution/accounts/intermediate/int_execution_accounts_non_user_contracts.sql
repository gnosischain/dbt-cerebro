...
WITH contract_addresses AS (
    ...
    FROM {{ source('execution', 'contracts') }}
    WHERE contract_address IS NOT NULL

    UNION ALL

    SELECT lower(assumeNotNull(address)) AS address
    FROM {{ source('execution', 'code_diffs') }}
    WHERE address IS NOT NULL

    UNION ALL
    SELECT lower(address) AS address FROM {{ ref('tokens_whitelist') }}

    UNION ALL
    SELECT lower(atoken_address) AS address FROM {{ ref('atoken_reserve_mapping') }}

    UNION ALL
    SELECT lower(supply_token_address) AS address
    FROM {{ ref('lending_market_mapping') }} WHERE supply_token_address != ''

    UNION ALL
    SELECT lower(stable_debt_token_address) AS address
    FROM {{ ref('lending_market_mapping') }} WHERE stable_debt_token_address != ''

    UNION ALL
    SELECT lower(variable_debt_token_address) AS address
    FROM {{ ref('lending_market_mapping') }} WHERE variable_debt_token_address != ''

    UNION ALL
    SELECT lower(pool_address) AS address
    FROM {{ ref('lending_market_mapping') }} WHERE pool_address != ''

    UNION ALL
    SELECT lower(address) AS address FROM {{ ref('contracts_whitelist') }}

    UNION ALL
    SELECT lower(pool_address) AS address FROM {{ ref('stg_pools__v3_pool_registry') }}
)
...
FROM (
    ...
    WHERE address IS NOT NULL
      AND address NOT IN (SELECT lower(address) FROM {{ ref('contracts_safe_registry') }})
