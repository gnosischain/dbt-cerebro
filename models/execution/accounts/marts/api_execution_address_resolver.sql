{{
    config(
        materialized='view',
        tags=['production', 'execution', 'accounts', 'tier1', 'api:address_resolver', 'granularity:latest'],
        meta={
            "api": {
                "methods": ["GET", "POST"],
                ...
                "parameters": [
                    {
                        ...
                        "type": "string",
                        "case": "lower",
                        "description": "Gnosis Chain address (0x…). Returns one row with boolean flags + counts for each account-portfolio domain."
                    }
                ],
                "pagination": {
                    "enabled": true,
