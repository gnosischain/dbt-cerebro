{{
  config(
    ...
    order_by='(search_key, result_type, address)',
    settings={ 'allow_nullable_key': 1 },
    tags=['production', 'execution', 'accounts', 'portfolio', 'search', 'granularity:latest']
  )
}}
...
WITH profile AS (
  ...
  FROM {{ ref('fct_execution_account_profile_latest') }}
