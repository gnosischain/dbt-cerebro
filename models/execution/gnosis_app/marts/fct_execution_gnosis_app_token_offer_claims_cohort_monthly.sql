{{
  config(
    ...
    tags=['production','execution','gnosis_app','token_offers','claims','retention','mart']
  )
}}
...
monthly_claim AS (
    SELECT
        ...
        sum(amount_received_usd)             AS amount_usd
    FROM {{ ref('int_execution_gnosis_app_token_offer_claims') }}
...
cohort_activity AS (
    ...
    FROM first_claim f
    INNER JOIN monthly_claim a ON f.address = a.address
