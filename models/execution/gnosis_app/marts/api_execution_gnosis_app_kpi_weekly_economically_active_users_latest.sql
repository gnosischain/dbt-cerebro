{{
  config(
    ...
    tags=['production','execution','gnosis_app','kpi','tier0',
          'api:gnosis_app_kpi_weekly_economically_active_users','granularity:last_week']
  )
}}

SELECT sub.*, (SELECT toDate(max(week)) FROM {{ ref('fct_execution_gnosis_app_weekly_economically_active_users') }}) AS as_of_date
FROM (
...
WITH weeks AS (
    SELECT week, cnt
    FROM {{ ref('fct_execution_gnosis_app_weekly_economically_active_users') }}
