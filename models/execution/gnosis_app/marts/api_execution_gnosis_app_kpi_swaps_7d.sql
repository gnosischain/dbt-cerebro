{{
  config(
    ...
    tags=['production','execution','gnosis_app','kpi','tier0',
          'api:gnosis_app_kpi_swaps','granularity:last_7d']
  )
}}

SELECT sub.*, (SELECT toDate(max(date)) FROM {{ ref('fct_execution_gnosis_app_swaps_daily') }}) AS as_of_date
FROM (
WITH days AS (
    SELECT date, n_swaps FROM {{ ref('fct_execution_gnosis_app_swaps_daily') }}
),
