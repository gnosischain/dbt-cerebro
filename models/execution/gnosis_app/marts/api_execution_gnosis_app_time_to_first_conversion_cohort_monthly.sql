{{
  config(
    materialized='view',
    tags=['production','execution','gnosis_app','tier1','api:gnosis_app_time_to_first_conversion_cohort','granularity:monthly']
  )
}}
...
WITH long_form AS (
    ...
    FROM {{ ref('int_execution_gnosis_app_first_conversion') }}
    UNION ALL
    SELECT
        toStartOfMonth(first_seen_at)                                        AS cohort_month,
