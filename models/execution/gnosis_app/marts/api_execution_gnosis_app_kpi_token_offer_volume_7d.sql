{{
  config(
    ...
    tags=['production','execution','gnosis_app','kpi','tier0',
          'api:gnosis_app_kpi_token_offer_volume','granularity:last_7d']
  )
}}

SELECT sub.*, (SELECT toDate(max(date)) FROM {{ ref('fct_execution_gnosis_app_token_offer_claims_daily') }}) AS as_of_date
FROM (
WITH days AS (
    SELECT date, sum(volume_received_usd) AS volume_usd
    FROM {{ ref('fct_execution_gnosis_app_token_offer_claims_daily') }}
    GROUP BY date
),
recent AS (SELECT sum(volume_usd) AS v FROM days
           WHERE date >= today() - INTERVAL 7 DAY AND date < today()),
prior  AS (SELECT sum(volume_usd) AS v FROM days
