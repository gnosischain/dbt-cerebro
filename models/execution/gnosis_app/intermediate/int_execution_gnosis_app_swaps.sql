...
cometh_presignatures AS (
    ...
    INNER JOIN {{ source('execution','transactions') }} tx
...
trade_rollup AS (
    ...
    WHERE block_timestamp >= toDateTime('2025-11-12')
      {% if start_month and end_month %}
        AND toStartOfMonth(block_timestamp) >= toDate('{{ start_month }}') - INTERVAL 1 MONTH
        AND toStartOfMonth(block_timestamp) <= toDate('{{ end_month }}')   + INTERVAL 2 MONTH
      {% endif %}
