...
rule_safe_invitation AS (
    ...
    FROM cometh_txs ct
    INNER JOIN {{ ref('int_execution_safes') }} s
        ON s.tx_hash = concat('0x', ct.transaction_hash)
    INNER JOIN {{ ref('int_execution_safes_module_events') }} m
...
rule_app_fee_erc20 AS (
    ...
    WHERE replaceAll(lower(lg.topic0), '0x', '') = 'ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'
      ...
      {% if start_month and end_month %}
        AND toStartOfMonth(toDate(lg.block_timestamp)) >= toDate('{{ start_month }}')
        AND toStartOfMonth(toDate(lg.block_timestamp)) <= toDate('{{ end_month }}')
      {% else %}
        {{ apply_monthly_incremental_filter('lg.block_timestamp', 'block_timestamp', add_and=True) }}
      {% endif %}
),
...
rule_invite_human AS (
    ...
    INNER JOIN cometh_txs ct
        ON he.transaction_hash = ct.transaction_hash
    INNER JOIN {{ ref('int_execution_circles_v2_inviter_canonical') }} ic
...
rule_profile AS (
    ...
    FROM {{ ref('contracts_circles_v2_NameRegistry_events') }} nre
    INNER JOIN cometh_txs ct
