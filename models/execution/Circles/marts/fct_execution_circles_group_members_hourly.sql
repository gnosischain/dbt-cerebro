...
membership_changes AS (
    SELECT
        group_address,
        toStartOfHour(toDateTime(start_ts)) AS timestamp,
        1 AS delta
    ...
    SELECT
        group_address,
        toStartOfHour(toDateTime(end_ts)) AS timestamp,
        -1 AS delta
    FROM trust_intervals
...
range_per_group AS (
    SELECT
        ...
        toStartOfHour(toDateTime({{ circles_chain_now_ts() }})) AS max_ts
    FROM changes_hourly
    GROUP BY 1
),
calendar AS (
    SELECT
        group_address,
        toDateTime(min_ts) + number * 3600 AS timestamp
    FROM range_per_group
    ARRAY JOIN range(dateDiff('hour', min_ts, max_ts) + 1) AS number
),
dense AS (
    SELECT
        c.group_address,
        c.timestamp,
        coalesce(ch.delta, 0) AS delta
    FROM calendar c
    LEFT JOIN changes_hourly ch
        ON c.group_address = ch.group_address
       AND c.timestamp = ch.timestamp
)
