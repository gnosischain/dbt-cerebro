{{
    config(
        materialized='view',
        tags=['production', 'execution', 'tier0', 'api:circles_v2_kpi_depositors_in_backers_pct', 'granularity:latest']
    )
}}
...
WITH depositors AS (
    SELECT DISTINCT backer FROM {{ ref('int_execution_circles_v2_backing_depositors_current') }}
),
backers AS (
    SELECT DISTINCT backer FROM {{ ref('int_execution_circles_v2_backers_current') }}
),
overlap AS (
    SELECT
