{{
    config(
        materialized='table',
        engine='MergeTree()',
        order_by='(withdrawal_credentials, validator_index)',
        tags=["production", "consensus", "fct:validators_explorer", "granularity:latest"]
    )
}}
...
FROM {{ ref('fct_consensus_validators_status_latest') }} s
