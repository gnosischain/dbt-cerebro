...
SELECT
    ...
    -- amount is emitted; daily_rate/apy are unaffected ratios (mGNO cancels).
    ,i.balance_gno / 32 AS balance_gno
    ,i.balance_prev_gno / 32 AS balance_prev_gno
    ,i.effective_balance_gno / 32 AS effective_balance_gno
    ,i.deposits_amount_gno / 32 AS deposits_amount_gno
    ,i.deposits_count AS deposits_count
    ,i.withdrawals_amount_gno / 32 AS withdrawals_amount_gno
    ,i.withdrawals_count AS withdrawals_count
    ,i.consolidation_inflow_gno / 32 AS consolidation_inflow_gno
    ,i.consolidation_outflow_gno / 32 AS consolidation_outflow_gno
    ,i.consensus_income_amount_gno / 32 AS consensus_income_amount_gno
    ...
    ,i.cumulative_consolidation_inflow_gno / 32 AS cumulative_consolidation_inflow_gno
    ,i.cumulative_consolidation_outflow_gno / 32 AS cumulative_consolidation_outflow_gno
    ,i.total_income_estimated_gno / 32 AS total_income_estimated_gno
    ,COALESCE(p.proposed_blocks_count, 0) AS proposed_blocks_count
    ,COALESCE(p.proposer_reward_total_gno, 0) / 32 AS proposer_reward_total_gno
    ,COALESCE(p.proposer_reward_attestations_gno, 0) / 32 AS proposer_reward_attestations_gno
    ,COALESCE(p.proposer_reward_sync_aggregate_gno, 0) / 32 AS proposer_reward_sync_aggregate_gno
    ,COALESCE(p.proposer_reward_proposer_slashings_gno, 0) / 32 AS proposer_reward_proposer_slashings_gno
    ,COALESCE(p.proposer_reward_attester_slashings_gno, 0) / 32 AS proposer_reward_attester_slashings_gno
