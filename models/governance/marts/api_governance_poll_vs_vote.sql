{{
  config(
    materialized='view',
    tags=['production','governance','tier2','api:governance_poll_vs_vote','granularity:latest']
  )
}}
...
SELECT
    ...
    {{ governance_phase_bucket('p.poll_posted_at', 'd.created_at', 'd.start_at', 'd.end_at') }} AS poll_phase,
    (
        p.is_temperature_check
        AND {{ governance_phase_bucket('p.poll_posted_at', 'd.created_at', 'd.start_at', 'd.end_at') }} = 'pre_discussion'
    ...
    (SELECT toDate(max(poll_posted_at)) FROM {{ ref('int_governance_poll_sentiment') }}) AS as_of_date

FROM {{ ref('int_governance_poll_sentiment') }} AS p
INNER JOIN {{ ref('int_governance_proposal_topic_links') }} AS l
    ON l.topic_id = p.topic_id
INNER JOIN {{ ref('api_governance_proposal_direction') }} AS d
