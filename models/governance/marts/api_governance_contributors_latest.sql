{{
  config(
    materialized='view',
    tags=['production','governance','tier2','api:governance_contributors','granularity:latest']
  )
}}
...
WITH eligible_likes AS (
    ...
    FROM {{ ref('stg_governance__forum_likes') }}
    WHERE hidden = 0 AND deleted = 0
      AND topic_id IN (SELECT id FROM {{ ref('stg_governance__forum_topics') }})
      AND post_id IN (SELECT id FROM {{ ref('stg_governance__forum_posts') }})
),
...
likes_received AS (
    ...
    INNER JOIN {{ ref('stg_governance__forum_posts') }} AS po ON po.id = l.post_id
    WHERE po.user_id > 0
    GROUP BY po.user_id
)
...
SELECT
    ...
    (SELECT toDate(max(created_at)) FROM {{ ref('stg_governance__forum_posts') }}) AS as_of_date
FROM (
    SELECT
        {{ pseudonymize_address('toString(p.user_id)') }} AS contributor_key,
        p.posts AS posts,
    ...
    LEFT JOIN {{ ref('stg_governance__forum_users') }} AS u ON u.id = p.user_id
