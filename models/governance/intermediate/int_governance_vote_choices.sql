{{
  config(
    materialized='table',
    engine='MergeTree()',
    order_by='(proposal_id, voter, preference_rank)',
    tags=['production','governance','votes']
  )
}}
...
WITH v AS (
    SELECT
        ...
        p.start_at,
        p.end_at
    FROM {{ ref('stg_governance__snapshot_votes') }} AS s
...
SELECT
    ...
    if(
        choice_index >= 1 AND choice_index <= length(choices),
        {{ classify_choice_polarity('arrayElement(choices, toUInt32(greatest(choice_index, 1)))') }},
        'unknown'
    )                                                       AS polarity,
