{{
  config(
    ...
    order_by='(week, step, utm_campaign, utm_source, utm_medium)',
...
--   card_ordered      Mixpanel 'Order the card' (seed-driven)
...
WITH signups AS (
    ...
    FROM {{ ref('int_mixpanel_ga_user_acquisition') }}
),
...
steps AS (
    ...
    SELECT
        ...
        metric                                  AS step,
        multiIf(metric = 'card_ordered', 1,
                metric = 'circles_created', 2,
                metric = 'crc_minted', 3, 9)    AS step_order,
    ...
    SELECT
        toStartOfWeek(first_date, 1)            AS week,
        event_type                              AS step,
        if(event_type = 'funded', 4, 5)         AS step_order,
        first_touch_campaign, first_touch_source, first_touch_medium
