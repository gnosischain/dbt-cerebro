...
{{
  config(
    ...
    order_by='(month, wallet_address)',
    partition_by='month',
    settings={ 'allow_nullable_key': 1 },
    tags=['production','celo','gpay','native','funder_wallet','tenure'],
    pre_hook=["SET join_use_nulls = 1"],
    post_hook=["SET join_use_nulls = 0"]
  )
}}

{# celo_execution begins at the OP-stack L2 migration block 31,056,500. There is no
   pre-migration Celo L1 history in this warehouse, so this floor is the hard edge of
   what can ever be known here, not a scoping choice we could relax. #}
{% set coverage_start = '2025-03-01' %}
