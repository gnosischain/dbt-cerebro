{{
  config(
    ...
    tags=['production','celo','gpay','funder_wallet','tenure']
  )
}}

{# The OP-stack L2 migration, block 31,056,500 — the first block celo_execution holds.
   Nothing before it exists in this warehouse at any price. #}
{% set coverage_start = '2025-03-26' %}
{# A wallet first observed inside this grace window is indistinguishable from one that
   had been transacting on Celo L1 for years: our window simply opens on top of it. #}
{% set censoring_grace_days = 7 %}
...
-- starts at the L2 migration ({{ coverage_start }}); Celo's L1 history is not in this
...
WITH presence AS (
    ...
    FROM {{ ref('int_celo_gpay_funder_wallet_monthly_presence') }}
...
joined AS (
    ...
    FROM {{ ref('int_celo_gpay_funder_wallets') }} w
    ...
    SELECT
        *,
        ...
        toUInt8(first_seen_at IS NOT NULL
                AND first_seen_at < toDate('{{ coverage_start }}')
                                   + {{ censoring_grace_days }})      AS is_left_censored
    FROM joined
)
