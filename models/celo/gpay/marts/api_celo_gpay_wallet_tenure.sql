{{
  config(
    materialized='view',
    tags=['production', 'celo', 'gpay', 'tier1', 'api:celo_gpay_wallet_tenure', 'granularity:latest']
  )
}}
...
SELECT
    ...
    (SELECT max(first_funded_at) FROM {{ ref('fct_celo_gpay_wallet_tenure') }}) AS as_of_date
FROM {{ ref('fct_celo_gpay_wallet_tenure') }}
