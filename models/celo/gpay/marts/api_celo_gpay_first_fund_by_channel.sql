{{
  config(
    materialized='view',
    tags=['production', 'celo', 'gpay', 'tier1', 'api:celo_gpay_first_fund_by_channel', 'granularity:latest']
  )
}}
...
SELECT
    ...
    count()            AS value,
    (SELECT max(observed_through) FROM {{ ref('fct_celo_gpay_card_funnel') }}) AS as_of_date
