{{
  config(
    materialized='view',
    tags=['production', 'celo', 'gpay', 'tier1', 'api:celo_gpay_churn_rates', 'granularity:monthly'],
    meta={
      "api": {
        "methods": ["GET"],
        "allow_unfiltered": true,
        "parameters": [
          {"name": "scope", "column": "scope", "operator": "=", "type": "string", "description": "Churn scope"},
          {"name": "start_date", "column": "month", "operator": ">=", "type": "date", "description": "Inclusive start month"},
          {"name": "end_date", "column": "month", "operator": "<=", "type": "date", "description": "Inclusive end month"}
        ],
        "sort": [{"column": "month", "direction": "DESC"}]
      }
    }
  )
}}

SELECT
    scope,
    toString(month) AS month,
    churn_rate,
    retention_rate
FROM {{ ref('fct_celo_gpay_churn_monthly') }}
ORDER BY scope, month
