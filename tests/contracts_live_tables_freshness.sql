...
{{ config(tags=['production', 'contracts', 'live']) }}
{% set error_after_minutes = var('live_freshness_error_after_minutes', 60) %}

{% set timestamp_tables = [
    'contracts_UniswapV3_Pool_events_live',
    'contracts_BalancerV2_Vault_events_live',
    'contracts_BalancerV3_Vault_events_live',
    'contracts_Swapr_v3_AlgebraPool_events_live',
...
FROM {{ ref(t) }}
HAVING minutes_stale > {{ error_after_minutes }}
UNION ALL
{% endfor %}
...
FROM {{ ref('api_execution_live_trades_freshness') }}
HAVING minutes_stale > {{ error_after_minutes }}
