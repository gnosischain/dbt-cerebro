{{ config(severity='warn', tags=['production', 'data_quality', 'data_quality_daily', 'lending']) }}
...
WITH bal AS (
    ...
    WHERE date >= today() - {{ var('test_lookback_days', 7) }} - 1
...
diffs AS (
    ...
    FROM {{ ref('int_execution_lending_aave_diffs_daily') }}
    WHERE date >= today() - {{ var('test_lookback_days', 7) }} - 1
    GROUP BY date, protocol, reserve_address
),
...
WHERE date >= today() - {{ var('test_lookback_days', 7) }}
  AND rel_residual > {{ var('lending_scaled_conservation_rel_tol', 0.10) }}
